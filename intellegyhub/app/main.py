...
def create_app(options_path: Path | None = None, runtime: AppRuntime | None = None) -> FastAPI:
    ...
    async def lifespan(app: FastAPI):
        if app.state.runtime is None:
            ...
            LOGGER.info(
                "Starting v0.5.101 chip=%s led=%s active_low=%s fn1_gpio=27 fn2_gpio=%s active_low=%s bias=%s debounce_ms=%s startup_buzzer=%s startup_buzzer_frequency=%s startup_buzzer_duration_ms=%s carrier_monitoring_poll_interval_seconds=%s onewire_bridge1_poll_interval_seconds=%s onewire_bridge2_poll_interval_seconds=%s mock=%s port=8098",
                config.chip_path,
                config.led_gpio,
    ...
    app = FastAPI(title="IntellegyHUB", version="0.5.101", lifespan=lifespan)
...
<body>
    ...
    <section class="extension-panel overview-panel">
      <div class="overview-identity">
        ...
        <dl class="overview-facts">
          ...
          <dt>Software</dt>
          <dd id="carrier-identity-software">0.5.101</dd>
        </dl>
  ...
  <script>
    ...
    function paintCarrier(carrier, appInfo = latestAppInfo) {
      ...
      document.getElementById('carrier-identity-software').textContent = identity.software_version || '0.5.101';
